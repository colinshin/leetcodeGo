...
这样做比较次数可能会减小，但是map对处理效率较低，实际测试没有得到优化，反而有些降低性能
...
func maxProduct2(words []string) int {
	...
	for _, v := range words {
		...
		if len(v) > maskLenInfo[mask] {
			/*
				一些单词具有相同的字符集,mask值也相同，则只保留这些单词中最长的一个单词即可。
				例如：单词集合（ab，aaaaabaabaaabb，bbabbabba）只保留单词aaaaabaabaaabb 即可。
			*/
